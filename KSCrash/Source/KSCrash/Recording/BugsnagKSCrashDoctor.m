...
@interface  BugsnagKSCrashDoctorParam: NSObject
...
@implementation BugsnagKSCrashDoctorParam
...
@interface BugsnagKSCrashDoctorFunctionCall: NSObject
...
@implementation BugsnagKSCrashDoctorFunctionCall
...
- (NSString*) descriptionForObjCCall
{
    ...
    BugsnagKSCrashDoctorParam* receiverParam = [self.params objectAtIndex:0];
    ...
    BugsnagKSCrashDoctorParam* selectorParam = [self.params objectAtIndex:1];
    if(![selectorParam.type isEqualToString:@BugsnagKSCrashMemType_String])
    {
        return nil;
    }
    ...
    for(int paramNum = 0; paramNum < paramCount; paramNum++)
    {
        ...
        if(paramNum < 2)
        {
            BugsnagKSCrashDoctorParam* param = [self.params objectAtIndex:(NSUInteger)paramNum + 2];
            if(param.value != nil)
            {
                if([param.type isEqualToString:@BugsnagKSCrashMemType_String])
                {
...
- (NSString*) descriptionWithParamCount:(int) paramCount
{
    ...
    {
        BugsnagKSCrashDoctorParam* param = [self.params objectAtIndex:(NSUInteger)i];
...
@implementation BugsnagKSCrashDoctor
...
{
    return [report objectForKey:@BugsnagKSCrashField_RecrashReport];
}

- (NSDictionary*) systemReport:(NSDictionary*) report
{
    return [report objectForKey:@BugsnagKSCrashField_System];
}

- (NSDictionary*) crashReport:(NSDictionary*) report
{
    return [report objectForKey:@BugsnagKSCrashField_Crash];
}

- (NSDictionary*) infoReport:(NSDictionary*) report
{
    return [report objectForKey:@BugsnagKSCrashField_Report];
}

- (NSDictionary*) errorReport:(NSDictionary*) report
{
    return [[self crashReport:report] objectForKey:@BugsnagKSCrashField_Error];
}

- (CPUFamily) cpuFamily:(NSDictionary*) report
{
    NSDictionary* system = [self systemReport:report];
    NSString* cpuArch = [system objectForKey:@BugsnagKSSystemField_CPUArch];
...
- (NSString*) mainExecutableNameForReport:(NSDictionary*) report
{
    ...
    return [info objectForKey:@BugsnagKSCrashField_ProcessName];
}

- (NSDictionary*) crashedThreadReport:(NSDictionary*) report
{
    NSDictionary* crashReport = [self crashReport:report];
    NSDictionary* crashedThread = [crashReport objectForKey:@BugsnagKSCrashField_CrashedThread];
    if(crashedThread != nil)
    {
        return crashedThread;
    }

    for(NSDictionary* thread in [crashReport objectForKey:@BugsnagKSCrashField_Threads])
    {
        if([[thread objectForKey:@BugsnagKSCrashField_Crashed] boolValue])
        {
            return thread;
        }
    }
...
{
    NSDictionary* backtrace = [threadReport objectForKey:@BugsnagKSCrashField_Backtrace];
    return [backtrace objectForKey:@BugsnagKSCrashField_Contents];
}

- (NSDictionary*) basicRegistersFromThreadReport:(NSDictionary*) threadReport
{
    NSDictionary* registers = [threadReport objectForKey:@BugsnagKSCrashField_Registers];
    NSDictionary* basic = [registers objectForKey:@BugsnagKSCrashField_Basic];
    return basic;
}
...
- (NSDictionary*) lastInAppStackEntry:(NSDictionary*) report
{
    ...
    {
        NSString* objectName = [entry objectForKey:@BugsnagKSCrashField_ObjectName];
...
{
    NSDictionary* machError = [errorReport objectForKey:@BugsnagKSCrashField_Mach];
    if(machError != nil)
    {
        NSString* exceptionName = [machError objectForKey:@BugsnagKSCrashField_ExceptionName];
        return [exceptionName isEqualToString:@"EXC_BAD_ACCESS"];
    }
    NSDictionary* signal = [errorReport objectForKey:@BugsnagKSCrashField_Signal];
    NSString* sigName = [signal objectForKey:@BugsnagKSCrashField_Name];
...
{
    NSDictionary* crashedThread = [self crashedThreadReport:report];
    NSArray* notableAddresses = [crashedThread objectForKey:@BugsnagKSCrashField_NotableAddresses];
    for(NSDictionary* address in [notableAddresses objectEnumerator])
    {
        NSString* type = [address objectForKey:@BugsnagKSCrashField_Type];
        if([type isEqualToString:@"string"])
        {
            NSString* value = [address objectForKey:@BugsnagKSCrashField_Value];
    ...
    {
        NSString* objectName = [entry objectForKey:@BugsnagKSCrashField_ObjectName];
        NSString* symbolName = [entry objectForKey:@BugsnagKSCrashField_SymbolName];
...
- (BugsnagKSCrashDoctorFunctionCall*) lastFunctionCall:(NSDictionary*) report
{
    BugsnagKSCrashDoctorFunctionCall* function = as_autorelease([[BugsnagKSCrashDoctorFunctionCall alloc] init]);
    NSDictionary* lastStackEntry = [self lastStackEntry:report];
    function.name = [lastStackEntry objectForKey:@BugsnagKSCrashField_SymbolName];

    NSDictionary* crashedThread = [self crashedThreadReport:report];
    NSDictionary* notableAddresses = [crashedThread objectForKey:@BugsnagKSCrashField_NotableAddresses];
    ...
    {
        BugsnagKSCrashDoctorParam* param = as_autorelease([[BugsnagKSCrashDoctorParam alloc] init]);
        ...
        {
            param.type = [notableAddress objectForKey:@BugsnagKSCrashField_Type];
            NSString* className = [notableAddress objectForKey:@BugsnagKSCrashField_Class];
            NSString* previousClass = [notableAddress objectForKey:@BugsnagKSCrashField_LastDeallocObject];
            NSString* value = [notableAddress objectForKey:@BugsnagKSCrashField_Value];

            if([param.type isEqualToString:@BugsnagKSCrashMemType_String])
            {
                param.value = value;
            }
            else if([param.type isEqualToString:@BugsnagKSCrashMemType_Object])
            {
                param.className = className;
                param.isInstance = YES;
            }
            else if([param.type isEqualToString:@BugsnagKSCrashMemType_Class])
            {
                param.className = className;
                param.isInstance = NO;
            }
...
- (NSString*) zombieCall:(BugsnagKSCrashDoctorFunctionCall*) functionCall
{
...
{
    NSDictionary* stack = [crashedThreadReport objectForKey:@BugsnagKSCrashField_Stack];
    return [[stack objectForKey:@BugsnagKSCrashField_Overflow] boolValue];
}

- (BOOL) isDeadlock:(NSDictionary*) report
{
    NSDictionary* errorReport = [self errorReport:report];
    NSString* crashType = [errorReport objectForKey:@BugsnagKSCrashField_Type];
    return [@BugsnagKSCrashExcType_Deadlock isEqualToString:crashType];
}
...
- (NSString*) diagnoseCrash:(NSDictionary*) report
{
    @try
    {
        NSString* lastFunctionName = [[self lastInAppStackEntry:report] objectForKey:@BugsnagKSCrashField_SymbolName];
        ...
        NSString* crashType = [errorReport objectForKey:@BugsnagKSCrashField_Type];
        if([crashType isEqualToString:@BugsnagKSCrashExcType_NSException])
        {
            NSDictionary* exception = [errorReport objectForKey:@BugsnagKSCrashField_NSException];
            NSString* name = [exception objectForKey:@BugsnagKSCrashField_Name];
            NSString* reason = [exception objectForKey:@BugsnagKSCrashField_Reason];
        ...
        BugsnagKSCrashDoctorFunctionCall* functionCall = [self lastFunctionCall:report];
        ...
        {
            uintptr_t address = (uintptr_t)[[errorReport objectForKey:@BugsnagKSCrashField_Address] unsignedLongLongValue];
            if(address == 0)
            {
    ...
    @catch (NSException* e)
    {
        ...
        if(symbols)
        {
            return [NSString stringWithFormat:@"No diagnosis due to exception %@:\n%@\nPlease file a bug report to the BugsnagKSCrash project.", e, symbols];
        }
        return [NSString stringWithFormat:@"No diagnosis due to exception %@\nPlease file a bug report to the BugsnagKSCrash project.", e];
    }
}
